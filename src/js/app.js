App = {
  ...
  //  database.empty();
    App.contracts.Adoption.deployed().then(function(obj) {
      obj.Mans(App.account).then(function(patient){
        var fname = patient[0];
        var lname = patient[1];
        var email = patient[2];
    ...
    obj.Mans2(App.account).then(function(patient1){
      var birthday = patient1[3];
      var gender = patient1[4];
      var occupation = patient1[5];
      var bloodgrp = patient1[6];
      var mrgstatus = patient1[7];

      $("#bday").html("Birthday: " + birthday);
      $("#gnder").html("Gender: " + gender);
      $("#occup").html("Occupation: " + occupation);
      $("#bldgrp").html("Blood Group: " + bloodgrp);
      $("#mrgst").html("Marital Status: " + mrgstatus);

    });

    obj.Mans3(App.account).then(function(patient2){
      var home = patient2[8];
      var current = patient2[9];
      var city = patient2[10];
      var state = patient2[11];
      var country = patient2[12];
      var phn = patient2[13];

      $("#haddr").html("Home Address: " + home);
      $("#caddr").html("Current Address: " + current);
      $("#cty").html("City: " + city);
      $("#sta").html("State: " + state);
      $("#cnry").html("Country: " + country);
      $("#phn").html("Phone No.: " + phn);
    });
